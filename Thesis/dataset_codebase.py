...
df_reprisk_id = pd.read_csv(r"C:\Users\minha\OneDrive\Courses\Thesis\RepRisk Identifiers.csv")
...
# Load PE universe to check sponsor-backed info
df_PE_universe = pd.read_excel(
    r"C:\Users\minha\OneDrive\Courses\Thesis\US PE Universe.xlsx", skiprows=4
).reset_index(drop=True).dropna(how="all")
pe_univ_comps = set(df_PE_universe["SP_ENTITY_NAME"].apply(clean_name))

# Extract sponsor-backed firm names from text using regex
pattern = re.compile(r"(?:Prior|Pending or Current) Sponsor-Backed\s*\[([^\]]*)\]")
def extract_all_companies(text):
    matches = pattern.findall(text)
    if matches:
        all_companies = []
        for group in matches:
            all_companies.extend([clean_name(c) for c in group.split(";")])
        return all_companies
    return []

# Apply sponsor extraction to original company universe
df_sponsor_info = df_companies_universe.copy()
df_sponsor_info["PE_sponsors"] = (
    df_sponsor_info["SPONSOR_BACKED_INFO"]
    .astype(str)
    .apply(extract_all_companies)
    .apply(lambda ll: [s for s in ll if s in pe_univ_comps])
)

# Collect all PE firms per company from year-wise data
all_companies_with_PE = []
for SP_ENTITY_ID, group in df_w_PE_status.groupby("SP_ENTITY_ID"):
    all_PE_firms = list(set([firm for firms in group["PE_Firm"] for firm in firms]))
    results = group.loc[group.index[0], ["SP_ENTITY_ID"]]
    results["all_PE_firms"] = all_PE_firms
    all_companies_with_PE.append(results)

df_all_companies_with_PE = pd.DataFrame(all_companies_with_PE)

# Cross-check sponsor-backed info with PE firm tracking
df_cross_check = df_all_companies_with_PE.merge(df_sponsor_info, on="SP_ENTITY_ID", how="inner")
tf = (df_cross_check["all_PE_firms"].apply(lambda x: x == [])) & (df_cross_check["PE_sponsors"].apply(lambda x: x != []))
df_cross_check[["SP_COMPANY_NAME", "all_PE_firms", "PE_sponsors"]].sample(100)

# Check consistency: firms marked as 'Never Sponsor-Backed' but with PE history
tf = df_cross_check["SPONSOR_BACKED_INFO"] == "Never Sponsor-Backed"
df_cross_check[tf]["all_PE_firms"].value_counts().sum()

# Load RepRisk index scores and clean
df_reprisk_index = pd.read_csv(r"C:\Users\minha\OneDrive\Courses\Thesis\RepRisk Index.csv")
...
df_incidents = pd.read_csv(r"C:\Users\minha\OneDrive\Courses\Thesis\RepRisk Incidents.csv")
